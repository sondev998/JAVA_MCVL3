...
public class b {
    ...
    public static void a(String string, int n2, byte[] byArray) {
        ...
        block4: {
            bl2 = true;
            string = RecordStore.apenRecordStore(string, true);
            recordEnumeration = string.enumerateRecords(null, null, false);
            byte[] byArray2 = new byte[]{};
            while (n2 > recordEnumeration.numRecords() + 1) {
                string.addRecord(byArray2, 0, byArray2.length);
            }
            while (recordEnumeration.hasNextElement()) {
                if (recordEnumeration.nextRecordId() != n2) continue;
                break block4;
            }
            bl2 = false;
        }
        if (bl2) {
            string.setRecord(n2, byArray, 0, byArray.length);
        } else {

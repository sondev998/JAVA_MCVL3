...
public abstract class MIDlet {
    ...
    public void setAppProperty(String string) {
        ...
        block5: {
            ...
            try {
                String string2;
                while ((string2 = MIDlet.a((InputStream)object)) != null) {
                    Serializable serializable;
                    int n2 = string2.indexOf(58);
                    if (n2 > 0) {
                        serializable = this.a;
                        string = string2.substring(0, n2);
                        ((Hashtable)serializable).put(string, string2.substring(n2 + 1).trim());
                        continue;
                    }
                    if ((string2 = string2.trim()).length() <= 0) continue;
                    Hashtable hashtable = this.a;
                    serializable = new StringBuffer();
                    hashtable.put(string, ((StringBuffer)serializable).append(this.a.get(string)).append(string2).toString());
                }
                break block5;
            }
